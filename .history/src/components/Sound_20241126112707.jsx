  ...
  return (
    <div className="fixed top-4 right-2.5 xs:right-4 z-50 group">
      ...
      <motion.button
        ...
        animate={{ scale: 1 }}
        transition={{ delay: 1 }}
        className="w-10 h-10 xs:w-14 xs:h-14 text-cyan-900 rounded-full flex items-center justify-center cursor-pointer z-50 p-2.5 xs:p-4 custom-bg hover-bn"
        aria-label={"Sound control button"}
        name={"Sound control button"}
      >
        {isPlaying ? (
          <Volume2
            className="w-full h-full text-cyan-500 group-hover:text-cyan-800"
            strokeWidth={1.5}
          />
        ) : (
          <VolumeX
